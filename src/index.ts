...
type SummaryType = 'pr' | 'issue' | 'commit' | 'release' | 'digest';

interface GithubSummaryInput {
  repo: string;
  type: SummaryType;
  sourceId: string;
  title: string;
...
interface GenerateGithubSummaryInput {
  repo: string;
  type: SummaryType;
  sourceId: string;
  title: string;
...
function docIdFor(input: Pick<GithubSummaryInput, 'repo' | 'type' | 'sourceId'>): string {
  return `${sanitizeIdPart(input.repo)}__${sanitizeIdPart(input.type)}__${sanitizeIdPart(input.sourceId)}`;
}

function isSummaryType(type: string): type is SummaryType {
...
function authFailed(req: { header: (name: string) => string | undefined }): boolean {
...
async function persistSummary(input: GithubSummaryInput): Promise<string> {
...
function parseModelText(data: unknown): string {
...
function extractSummaryPayload(rawText: string): Pick<GithubSummaryInput, 'summary' | 'keyPoints' | 'actionItems'> {
...
export const upsertGithubSummary = onRequest(
  { secrets: [GITHUB_SUMMARY_API_KEY], region: 'us-central1' },
  async (req, res) => {
    if (req.method !== 'POST') {
    ...
    const body = req.body as Partial<GithubSummaryInput>;
    ...
    if (
      ...
      res.status(400).json({
        error:
          'Invalid payload. Required: repo, type(pr|issue|commit|release|digest), sourceId, title, summary',
      });
      return;
    }

    const input: GithubSummaryInput = {
      repo: body.repo,
      type: body.type,
...
export const generateGithubSummary = onRequest(
  { secrets: [GITHUB_SUMMARY_API_KEY, AI_ACCESS_API_KEY, AI_ACCESS_BASE_URL], region: 'us-central1' },
  async (req, res) => {
    if (req.method !== 'POST') {
      res.status(405).json({ error: 'Method not allowed. Use POST.' });
      return;
    ...
    const body = req.body as Partial<GenerateGithubSummaryInput>;
    if (
      !body ||
      typeof body.repo !== 'string' ||
      typeof body.type !== 'string' ||
      ...
      res.status(400).json({
        error:
          'Invalid payload. Required: repo, type(pr|issue|commit|release|digest), sourceId, title, content',
      });
      return;
    }
